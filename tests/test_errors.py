...
@pytest.mark.asyncio
async def test_error_1(starknet):
    ...
    contract = await starknet.deploy(
        source="contracts/error_files/blake2f_error_1.cairo",
    )
...
@pytest.mark.asyncio
async def test_error_2(starknet):
    ...
    # Deploy contract
    contract = await starknet.deploy(
        source="contracts/error_files/blake2f_error_2.cairo",
    )
    LOGGER.info(f"> Deployed blake2f.cairo.")
